    ...
    def __init__(self, data_dir, label_path, data_type, data_ratio=100, opt=None,  use_syn=False, syn_data_dir=None, syn_label_path=None):
        ...
        if data_ratio == 100:
            with open(label_path, "rb") as f:
                ...
                new_tr_dl = []
                for data in tr_dl:
                    data['img_root'] = os.path.join(data_dir, data['img_root'])
                    new_tr_dl.append(data)
            self.data_list = new_tr_dl
        ...
        if self.use_syn:
            with open(syn_label_path, "rb") as f:
                tr_dl = pickle.load(f)
            new_tr_dl = []
            for data in tr_dl:
                data['img_root'] = os.path.join(syn_data_dir, data['img_root'])
                new_tr_dl.append(data)
            self.data_list.extend(new_tr_dl)

            print('Synthesised Samples:', len(new_tr_dl))
    ...
    def __init__(self, data_dir, label_path, data_type, data_ratio=100, opt=None, use_syn=False, syn_data_dir=None, syn_label_path=None):
        ...
        if data_ratio == 100:
            with open(label_path, "rb") as f:
                ...
                new_tr_dl = []
                for data in tr_dl:
                    data['img_root'] = os.path.join(data_dir, data['img_root']  + '.jpg')
                    new_tr_dl.append(data)
            self.data_list = new_tr_dl
            print('Total Real Samples:', len(self.data_list))
        else:
        ...
        real_class_num_list = []
        for i in range(5):
            real_class_num = len([data for data in new_tr_dl if int(data['label']) == i])
            real_class_num_list.append(real_class_num)
            print(f'Real Class {i} Number : {real_class_num}')
        ...
        if self.use_syn:
            ...
            syn_label_bal_path = 'GenerateDataBalance/IDRiD/train.pkl'
            syn_balance_data_dir = '/cpfs01/projects-HDD/neikiuyiliaodamoxing_HDD/sunyuqi/SDGenerateDownStreamDatasetBalance'
            with open(syn_label_bal_path, "rb") as f:
                tr_dl = pickle.load(f)
            for data in tr_dl:
                data['img_root'] = os.path.join(syn_balance_data_dir, data['img_root'])
                new_tr_dl.append(data)

            max_class_num = len([data for data in self.data_list if int(data['label']) == 0]) * 5
            syn_class_list = []
            for i in range(5):
                syn_class = [data for data in new_tr_dl if int(data['label']) == i]
                syn_class_list.append(syn_class)
            
            tr_dl = []
            for i in range(5):
                syn_number = max_class_num - real_class_num_list[i]
                tr_dl.extend(syn_class_list[i][:syn_number])
                print(f'Syn Class {i} Number : {syn_number}')
    ...
    def __init__(self, data_dir, label_path, data_type, data_ratio=100, opt=None, use_syn=False, syn_data_dir=None, syn_label_path=None):
        ...
        if data_ratio == 100:
            with open(label_path, "rb") as f:
                ...
                new_tr_dl = []
                for data in tr_dl:
                    data['img_root'] = os.path.join(data_dir, data['img_root'] + '.png') 
                    new_tr_dl.append(data)
            self.data_list = new_tr_dl
            print('Total Real Samples:', len(self.data_list))
        else:
